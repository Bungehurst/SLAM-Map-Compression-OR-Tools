...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        cerr << endl << "Usage: ./stereo_kitti path_to_vocabulary path_to_settings path_to_sequence path_to_map" << endl;
        return 1;
    }
    ...
    SLAM.LoadMap(string(argv[4]), string(argv[2]));
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        cv::Mat Tcw = SLAM.LocalizeStereo(imLeft, imRight);

...
namespace ORB_SLAM2 {
    ...
    void Map::CompressByGrid2D() {
        ...
        std::vector<MPVariable*> vx;
        vx.reserve(mspMapPoints.size());
       
        int index = 0;
        for (MapPoint *pMP: mspMapPoints) {
            pMP->mnIndexForCompression = index;
            vx[index] = solver->MakeIntVar(0,1,"x"); 
            index++;
        ...
        MPObjective* const expr_objective = solver->MutableObjective();
        index = 0;
        for (MapPoint *pMP: mspMapPoints) {
            float coef = (float) maxObservations - pMP->Observations();
            expr_objective->SetCoefficient(vx[index],coef);
            index++;
        }
...
//        fGridWidth = 1.0 / fGridWidth;
        ...
        for (KeyFrame *pKF: mspKeyFrames) {
            ...
            MPConstraint* const expr_score = solver->MakeRowConstraint(b,infinity,"expr_score");
            for (MapPoint *pMP: sMPs) {
                ...
                expr_score->SetCoefficient(vx[pMP->mnIndexForCompression],1);
            }

            for (const auto &g: grids) {

                set<long unsigned int> indexes = g.second;
                MPVariable* const th = solver->MakeIntVar(0, 1, "th");
                MPConstraint* const expr_cons = solver->MakeRowConstraint(1,infinity,"expr_cons");
                expr_cons->SetCoefficient(th,1);
                for (long unsigned int i: indexes) {
                    expr_cons->SetCoefficient(vx[i],1);
                }
                expr_objective->SetCoefficient(th,lambda_grid);
            }
            
            MPVariable* const th = solver->MakeIntVar(0, b, "th");
            expr_score->SetCoefficient(th,1);
            expr_objective->SetCoefficient(th,lambda);
        }
        LOG(INFO) << "Number of constraints = " << solver->NumConstraints();
        expr_objective->SetMinimization();
        ...
        LOG(INFO) << "Solution:";
        LOG(INFO) << "Objective value = " << expr_objective->Value();
        ...
        for (MapPoint *pMP: mspMapPoints) {
            ...
            double keep = vx[i]->solution_value();
            if (keep > 0) {
                unique_lock<mutex> lock(mMutexMap);
    ...
    void Map::CompressByGrid3D() {
        ...
        // Create the mip solver with the GLOP backend.
        std::unique_ptr<MPSolver> solver(MPSolver::CreateSolver("GLOP")); //SCIP
        if (!solver) {
            LOG(WARNING) << "GLOP solver unavailable.";
            return;
        }// [END solver]
...
//        fGridWidth = 1.0 / fGridWidth;
        ...
        for (auto cube: mCubeMap) {
            ...
            if (sMPInCube.size() < b2)
                continue;
            MPConstraint* const expr_cons2 = solver->MakeRowConstraint(b2,infinity,"expr_cons2");
            for (long unsigned int id: sMPInCube) {
                MapPoint *pMP = mID_MPs[id];
                expr_cons2->SetCoefficient(vx[pMP->mnIndexForCompression],1);
            }
            MPVariable* const th = solver->MakeIntVar(0, b2, "th");
            expr_cons2->SetCoefficient(th,1);
            expr_objective->SetCoefficient(th,lambda_space);
        }
        mCubeMap.clear();

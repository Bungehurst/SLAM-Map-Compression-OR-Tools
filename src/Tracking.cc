    ...
    cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp) {
        ...
        if (mImGray.channels() == 3) {
            if (mbRGB) {
                ...
                cvtColor(imGrayRight, imGrayRight, COLOR_RGB2GRAY);
            } else {
                cvtColor(mImGray, mImGray, COLOR_BGR2GRAY);
                cvtColor(imGrayRight, imGrayRight, COLOR_BGR2GRAY);
            }
        } else if (mImGray.channels() == 4) {
            if (mbRGB) {
                cvtColor(mImGray, mImGray, COLOR_RGBA2GRAY);
                cvtColor(imGrayRight, imGrayRight, COLOR_RGBA2GRAY);
            } else {
                cvtColor(mImGray, mImGray, COLOR_BGR2GRAY);
                cvtColor(imGrayRight, imGrayRight, COLOR_BGR2GRAY);
            }
...
//            cerr << "vpCandidateKFs is empty" << endl;
        ...
        for (int i = 0; i < nKFs; i++) {
            ...
            if (pKF->isBad())
                ...
                const vector<KeyFrame *> vNeighs = pKF->GetBestCovisibilityKeyFrames(2);
                for (vector<KeyFrame *>::const_iterator itNeighKF = vNeighs.begin(), itEndNeighKF = vNeighs.end();
                     itNeighKF != itEndNeighKF; itNeighKF++) {
                    KeyFrame *pNeighKF = *itNeighKF;
                    if (!pNeighKF->isBad()) {
                        nmatches += matcher.SearchByBoWMore(pNeighKF, mCurrentFrame, vvpMapPointMatches[i]);
                    }
                }
                if (nmatches < 15) { // 15
                    vbDiscarded[i] = true;
    ...
    cv::Mat Tracking::LocalizeImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD) {
        mImGray = imRGB;
        cv::Mat imDepth = imD;
        ...
        if (!bOK) {
//            cout << "The frame cannot be localized in the map!" << endl;
        } else {
            bOK = TrackLocalMap();
            if (!bOK) {
//                cout << "Failed in tracking local map!" << endl;
            }
        }
        // Update drawer
        mpFrameDrawer->Update(this);
        ...
        if (bOK) {
            cv::Mat Twc = cv::Mat::eye(4, 4, mCurrentFrame.mTcw.type());
            mCurrentFrame.mRwc.copyTo(Twc.rowRange(0, 3).colRange(0, 3));
            mCurrentFrame.mOw.copyTo(Twc.rowRange(0, 3).col(3));
            mpMapDrawer->AddLocalizedFrame(Twc, true);
        } else {
    ...
    cv::Mat Tracking::LocalizeImageStereo(const cv::Mat &imLeft, const cv::Mat &imRight) {
        mImGray = imLeft;
        ...
        mCurrentFrame = Frame(mImGray, imGrayRight, 0.0, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary,
                              mK, mDistCoef, mbf, mThDepth);
